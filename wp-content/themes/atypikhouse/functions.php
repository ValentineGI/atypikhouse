...
if ( ! function_exists( 'atypikhouse_setup' ) ) :
	...
	function atypikhouse_setup() {
		 ...
		 * to change 'so' to the name of your theme in all the template files.
		 */
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'atypikhouse_custom_background_args',
				array(
 ...
 */
function atypikhouse_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'atypikhouse_content_width', 640 );
}
add_action( 'after_setup_theme', 'atypikhouse_content_width', 0 );
 ...
 */
function atypikhouse_widgets_init() {
	register_sidebar(
		array(
...
add_action( 'widgets_init', 'atypikhouse_widgets_init' );
