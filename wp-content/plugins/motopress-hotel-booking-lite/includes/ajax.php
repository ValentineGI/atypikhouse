...
class Ajax {
						...
						?><tr>
							<td>
								...
								if ( $displayName ) {
									...
									echo esc_html( sprintf( __( '%1$s on %2$s', 'motopress-hotel-booking' ),
										$displayName,
										$noteDate
									) );
								} else {
	...
	public function get_free_accommodations_amount(){
		...
		if ( MPHB()->settings()->main()->getDirectBookingPricing() != 'disabled' ) {
			...
			$priceHtml = apply_filters( 'mphb_tmpl_the_room_type_price_for_dates',
